...
test("存在する投稿IDを指定すると対応する投稿が取得できること", async () => {
  ...
  (mockPostRepository.findById as ReturnType<typeof vi.fn>).mockResolvedValue(ok(mockPost));
...
test("リポジトリでエラーが発生した場合はそのエラーがそのまま返されること", async () => {
  ...
  (mockPostRepository.findById as ReturnType<typeof vi.fn>).mockResolvedValue(err(repositoryError));
