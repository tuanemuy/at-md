import { pgTable, text, uuid, varchar, boolean, timestamp, pgEnum } from "drizzle-orm/pg-core";
import { commonColumns, foreignKeyOptions } from "./common";
import { users } from "./users";
import { documentScopeSchema } from "@/domain/document/models/document";

// 文書公開範囲の列挙型
export const documentScope = pgEnum("document_scope", documentScopeSchema.options);
...
export const documents = pgTable("documents", {
  ...commonColumns,
  gitHubRepoId: uuid("github_repo_id").notNull().references(() => githubRepos.id, foreignKeyOptions),
  userId: uuid("user_id").notNull().references(() => users.id, foreignKeyOptions),
...
export const documentTags = pgTable("document_tags", {
  ...
  tagId: uuid("tag_id").notNull().references(() => tags.id, foreignKeyOptions),
}); 